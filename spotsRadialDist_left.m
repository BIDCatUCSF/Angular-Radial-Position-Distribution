...
%          <Item name="Bikem Find Spots Close To Surface v3 Left" icon="Matlab" tooltip="Find spots close to surface.">
...
for vSurfaceIndex = 1:vNumberOfSurfacesSelected
    ...
    for vSpotsIndex = 1:vNumberOfSpotsSelected
        ...
        %mkdir(strcat('D:\Data\Bikem\analysis_results\', savedir))
        mkdir(strcat(savedirpath, '\', savedir))
        
        
     %fname = char(vSpots.GetName);
        ...
        rads = 0:0.05:2.1*pi;
        %rhos = zeros(1,length(rads))+radcurv;
        
        grey = [0.5 0.5 0.5];
        thetas = atan2(yys, xxs) + pi;
        saveas(gcf, strcat(strcat(savedirpath, '\', savedir),strcat(fname, '_distance_from_polar_origin')), 'jpeg')
        ...
        set(ggl, 'color', grey);
        hold on
        %gg2 = polar(rads, rhos-100, '-');
        %et(gg2, 'color', grey);
        %hold on
        %gg3 = polar(rads, rhos+100, '-');
        %set(gg3, 'color', grey);
        %hold on
        ...
        saveas(gcf, strcat(strcat(savedirpath, '\', savedir), strcat(fname,'_polar_overlay')), 'jpeg')
        adamangs =[];
        for i = 0:ntheta
            ang = i*2*pi/ntheta;
        ...
        ylabel('Number/Bin')
        title(char(vSpots.GetName))
        saveas(gcf, strcat(strcat(savdirpath, '\', savedir), strcat(fname, '_angle_from_center')), 'jpeg')
        ...
        ylabel('Number/Bin')
        title(char(vSpots.GetName))
        saveas(gcf, strcat(strcat(savedirpath, '\',  savedir), strcat(fname, '_arc_distance_from_center')), 'jpeg')
        
        
        xlswrite(strcat(strcat(savedirpath, '\',  savedir, savedir),strcat(fname, '_polardistance.xls')), ...
            [px;py]', 'polar_distance');
        xlswrite(strcat(strcat(savedirpath, '\',  savedir, savedir),strcat(fname), '_anteriorangles.xls'), ...
            [xchunknz;totalchunksnz]', 'anterior_angles');
        
        xlswrite(strcat(strcat(savedirpath, '\',  savedir, savedir),strcat(fname), '_anteriorangles_binned.xls'), ...
            [nn;xouty]', 'anterior_angles_binned');
        
        xlswrite(strcat(strcat(savedirpath, '\',  savedir, savedir),strcat(fname), '_DV.xls'), ...
            [vv;uu]', '3binnedbyz');
